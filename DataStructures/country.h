...
public:
    ...
    void add_edge(dynamic_array< dynamic_array<edge> >& graph, int64_t from, int64_t to, int64_t capacity, double cost);
    bool assign_dwarfs_to_workplaces(const std::string& output_file);

    bool spfa(dynamic_array<dynamic_array<edge>>& graph, int64_t source, int64_t sink, dynamic_array<double>& dist, dynamic_array<int64_t>& parent_v, dynamic_array<int64_t>& parent_e);

    double min_cost_max_flow(dynamic_array<dynamic_array<edge>>& graph, int64_t source, int64_t sink);
