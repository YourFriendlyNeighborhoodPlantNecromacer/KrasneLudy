...
void generate_country(const std::string file_name, int64_t precision_point = 4, int64_t workplace_quantity_per_material = 10, int64_t house_quantity_per_material = 10, int64_t defult_capacity = 5, bool randomize_quantity = true){
    ...
    for(int64_t current_material = 0; current_material < NamedValues::material::size; current_material++){
        ...
        for(int64_t index = 0; index<current_material_count; index++){
            x = rand()%((int64_t)precision+1)/precision;
            y = rand()%((int64_t)precision+1)/precision;
            z = rand()%((int64_t)precision+1)/precision;
            if(randomize_quantity) capacity = rand()%(defult_capacity)+1;
